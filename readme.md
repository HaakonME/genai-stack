...
|------------------------|------------------------------------|-----------------------------------------------------------------------------|
| MODEL_REPO_BASE_URL    | http://host.docker.internal:11434  | REQUIRED - URL to Ollama, Hugging Face or other  LLM API                    |   
| NEO4J_URI              | neo4j://database:7687              | REQUIRED - URL to Neo4j database                                            |
| NEO4J_USERNAME         | neo4j                              | REQUIRED - Username for Neo4j database                                      |
| NEO4J_PASSWORD         | password                           | REQUIRED - Password for Neo4j database                                      |
| LLM                    | llama2                             | REQUIRED - Can be any Ollama or Hugging Face model tag, or gpt-4 or gpt-3.5 | 
...
## LLM Configuration
MacOS and Linux users can use any LLM that's available via Ollama or Hugging Face. Check the "tags" section under the model page you want to use on https://ollama.ai/library or https://huggingface.co/models and write the tag for the value of the environment variable `LLM=` in th e`.env` file.
...
**Linux**
No need to install model from Ollama or Hugging Face manually, it will run in a container as
part of the stack when running with the Linux profile: run `docker compose --profile linux up`.
Make sure to set the `MODEL_REPO_BASE_URL=http://llm:11434` in the `.env` file when using Model docker container.

NOTE: docker compose is the latest Docker version's syntax, some Linux distributions like Debian GNU/Linux Unstable still provides two separate commands, docker and docker-compose (with hyphen). start.sh contains an if statement to detect either docker compose or docker-compose.
